...
from config.roberta_config import INPUT_DIR, ORI_DATA_DIR, OUTPUT_DIR
from data_process.data_process import pp, preprocess_df
from datasets.datasets import SentimentBalanceSampler, TweetCollate, TweetDataset
from train_functions.loss_functions import define_criterion
from train_functions.roberta_models import build_model
from utils.helper import Logger, read_pickle, seed_everything, to_pickle
...
def train(train_df, CONFIG):
    ...
    folds = StratifiedKFold(
        n_splits=CONFIG["FOLD"], shuffle=True, random_state=CONFIG["SEED"]
    )
    for n_fold, (train_idx, valid_idx) in enumerate(
        folds.split(train_dataset.df["textID"], train_dataset.df["sentiment"])
    ):
        ...
        optimizer_grouped_parameters = [
            ## BERT param
            {
                "params": [
                    ...
                    if (not any(nd in n for nd in no_decay)) and (n in bert_params)
                ],
            ...
            ## Other param
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if (not any(nd in n for nd in no_decay)) and (n not in bert_params)
                ],
                "weight_decay": CONFIG["WEIGHT_DECAY"],
                "lr": CONFIG["LR"] * 1,
            },
            {
                "params": [
                    p
                    for n, p in param_optimizer
                    if (any(nd in n for nd in no_decay)) and (n not in bert_params)
                ],
                "weight_decay": 0.0,
                "lr": CONFIG["LR"] * 1,
            },
        ...
        while n_data < TOTAL_DATA:
            ...
            for batch in tqdm(train_loader):
                ...
                if (
                    (
                        (ep > 0)
                        & (n_batch % (int(5 * 32 / CONFIG["TRAIN_BATCH_SIZE"])) == 0)
                    )
                    | ((n_batch % (int(50 * 32 / CONFIG["TRAIN_BATCH_SIZE"])) == 0))
                    | (n_data >= TOTAL_DATA)
                ) and (CONFIG["SWA"] and ep > 0 and t_batch >= T_TOTAL * 0.50):
                ...
                if (
                    ...
                    trn_jac_text_neu = np.array(jac_text_list)[
                        np.array(jac_sentiment_list) == "neutral"
                    ].mean()
                    trn_jac_text_pos = np.array(jac_text_list)[
                        np.array(jac_sentiment_list) == "positive"
                    ].mean()
                    trn_jac_text_neg = np.array(jac_text_list)[
                        np.array(jac_sentiment_list) == "negative"
                    ].mean()
                    ...
                    val_jac_text_neu = val["jaccard_text"][
                        val["sentiment"] == "neutral"
                    ].mean()
                    val_jac_text_pos = val["jaccard_text"][
                        val["sentiment"] == "positive"
                    ].mean()
                    val_jac_text_neg = val["jaccard_text"][
...
def calc_jaccard(pred, batch, CONFIG):
    ...
    jaccard_token = (bi_pred * bi_targ == 1.0).sum(axis=1) / (
        bi_pred + bi_targ >= 1.0
    ).sum(axis=1)
    ...
    for i in range(bi_pred.shape[0]):
        ...
        s_text, e_text = (
            np.array(text_idx)[s_cl_text],
            np.array(text_idx)[e_cl_text - 1] + 1,
        )
        ...
        if CONFIG["PRE_PROCESS"]:
            ...
            jaccard_cl_text.append(
                word_base_jaccard(cl_selected_text, pp_cl_selected_text)
            )
            ...
            jaccard_cl_text.append(
                word_base_jaccard(cl_selected_text, cl_text[s_cl_text:e_cl_text])
            )
            pred_selected_text.append(text[s_text:e_text])
            pred_cl_selected_text.append(cl_text[s_cl_text:e_cl_text])
...
def create_valid(model, valid_loader, CONFIG):
    ...
    with torch.no_grad():
        for i, batch in enumerate(valid_loader):
            ...
            if CONFIG["BUCKET"]:
                pad = np.zeros(
                    (output.shape[0], CONFIG["MAX_LEN"] - output.shape[1], 2)
                )
                pad[:, :, :] = -1e10
            ...
            if i == 0:
                ...
                valid_jaccard_token = np.append(
                    valid_jaccard_token, jac["jaccard_token"], axis=0
                )
                valid_jaccard_cl_text = np.append(
                    valid_jaccard_cl_text, jac["jaccard_cl_text"], axis=0
                )
                valid_jaccard_text = np.append(
                    valid_jaccard_text, jac["jaccard_text"], axis=0
                )
